...
class Logger(Dispatcher):
    ...
    def unsqueeze(self, input, dim):
        ...
        print(f'{self.name} {r.t_name}: {self.size(r)} = {input.t_name}.unsqueeze({dim})')
        return r
    def squeeze(self, input, dim):
    ...
    def expand(self, input, sizes):
        ...
        print(f'{self.name} {r.t_name}: {self.size(r)} = {input.t_name}.expand({sizes})')
        return r
...
class Autograd(Dispatcher):
    ...
    def expand(self, input: Tensor, sizes: List[int]):
        ...
        def propagate(dL_doutputs: List[Tensor]):
            ...
            dims = tuple(i for i in range(self.inner.dim(input)) if input_size[i] != sizes[i])
    ...
    def squeeze(self, input: Tensor, dim):
        r = self.inner.squeeze(input, dim)
        print(f'{self.name} {r.t_name}: {self.size(r)} = {input.t_name}.squeeze(dim={dim})')
        def propagate(dL_doutputs: List[Tensor]):
            dL_dr, = dL_outputs
    ...
    def unsqueeze(self, input: Tensor, dim):
        r = self.inner.unsqueeze(input, dim)
        print(f'{self.name} {r.t_name}: {self.size(r)} = {input.t_name}.unsqueeze(dim={dim})')
        def propagate(dL_doutputs: List[Tensor]):
            dL_dr, = dL_doutputs
...
class Batched(Dispatcher):
    ...
    def mul(self, lhs, rhs):
        ...
        if not isinstance(rhs, float): assert self.inner.size(rhs)[0] == self.length
