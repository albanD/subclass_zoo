    ...
    class Autograd:
        ...
        def mul(self, lhs, rhs):
            ...
            def propagate(dL_doutputs: List[Tensor]):
                ...
                # self or Autograd???
                dL_dlhs = Autograd.backward_dispatch(
                    self, lambda s: s.mul(dL_dr, dr_dlhs)
                )
                dL_drhs = Autograd.backward_dispatch(
                    self, lambda s: s.mul(dL_dr, dr_drhs)
                )
        ...
        def sum(self, input: Tensor, name: Optional[str] = None):
            ...
            def propagate(dL_doutputs: List[Tensor]):
                ...
                size = input.size()
                return [
                    Autograd.backward_dispatch(self, lambda s: s.expand(dL_dr, size))
                ]

            Autograd.get_gradient_tape(self).append(
                TapeEntry(
                    inputs=[input.t_name], outputs=[r.t_name], propagate=propagate
                )
            )
            return r
            ...
            for entry in reversed(Autograd.get_gradient_tape(self)):
                ...
                for input, dL_dinput in zip(entry.inputs, dL_dinputs):
                    if input not in dL_d:
                        ...
                        dL_d[input] = Autograd.backward_dispatch(
                            self, lambda s: s.add(dL_d[input], dL_dinput)
                        )
...
class Example2Direct(Autograd, Torch):
    ...
    def run_gradients(self):
        ...
        L1 = self.sum(
            self.add(self.mul(dL0_da, dL0_da), self.mul(dL0_db, dL0_db)), name="L1"
        )
...
class Example2Indirect(Autograd2, Autograd1, Torch):
    ...
    def run_gradients(self):
        ...
        L1 = Autograd1.sum(
            ...
            Autograd1.add(
                self,
                Autograd1.mul(self, dL0_da, dL0_da),
                Autograd1.mul(self, dL0_db, dL0_db),
            ),
            name="L1",
        )
